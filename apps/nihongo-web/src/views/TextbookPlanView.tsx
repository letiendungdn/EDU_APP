...
import { useBookAudioFilesQuery, useKanjiLessonsQuery, useLessonsQuery } from '../hooks/queries';
import { JLPT_MIND_LEVELS, type JlptMindLevel } from '../data/jlpt-mind-map-shared';
import {
  JLPT_TEXTBOOK_SERIES,
  TEXTBOOK_AUDIO_MATCH,
  textbooksFor,
  type JlptTextbookSeries,
} from '../data/jlpt-textbooks';
import { PLAN_LEVELS, buildStudyPlan, type PlanAudio, type PlanTaskKind } from '../data/textbook-study-plans';
...
function loadProgress(key: string): Progress {
  try {
    ...
    return { done: Array.isArray(parsed.done) ? parsed.done : [], notes: parsed.notes ?? {} };
  } catch {
...
export default function TextbookPlanView({
  ...
  const [level, setLevel] = useState<JlptMindLevel>(
    PLAN_LEVELS[initialSeries].includes(initialLevel) ? initialLevel : PLAN_LEVELS[initialSeries][0],
  );

  // Đổi sách/cấp từ menu chỉ đổi URL (cùng trang) → đồng bộ lại state từ props.
  useEffect(() => {
    setSeries(initialSeries);
    setLevel(PLAN_LEVELS[initialSeries].includes(initialLevel) ? initialLevel : PLAN_LEVELS[initialSeries][0]);
  }, [initialSeries, initialLevel]);

  const { data: lessons = [], isLoading: lessonsLoading, isError: lessonsError } = useLessonsQuery();
  const { data: kanjiLessons = [], isLoading: kanjiLoading } = useKanjiLessonsQuery();
  ...
  const audio = useMemo<PlanAudio[]>(() => {
    const match = TEXTBOOK_AUDIO_MATCH[series];
    if (!match) return [];
  ...
  }, [bookAudio, series, level]);

  const plan = useMemo(
    () =>
      buildStudyPlan(series, level, {
    ...
    [series, level, lessons, kanjiLessons, readings, audio],
  );
  ...
  const select = (nextSeries: JlptTextbookSeries, nextLevel: JlptMindLevel) => {
    const lv = PLAN_LEVELS[nextSeries].includes(nextLevel) ? nextLevel : PLAN_LEVELS[nextSeries][0];
    setSeries(nextSeries);
    setLevel(lv);
    router.replace(`/textbooks?series=${nextSeries.toLowerCase()}&level=${lv.toLowerCase()}`, { scroll: false });
  };

  const meta = JLPT_TEXTBOOK_SERIES[series];
  const books = [...textbooksFor(level, 'GRAMMAR'), ...textbooksFor(level, 'VOCAB'), ...textbooksFor(level, 'KANJI')]
    .filter((b, i, arr) => b.series === series && arr.findIndex((x) => x.title === b.title) === i);
  const doneCount = plan ? plan.sections.flatMap((s) => s.units).filter((u) => progress.done.includes(u.id)).length : 0;
  const pct = plan && plan.unitCount ? Math.round((doneCount / plan.unitCount) * 100) : 0;
  const loading = lessonsLoading || kanjiLoading;
  const emptyPlan = plan && plan.sections.every((s) => s.units.every((u) => u.tasks.every((t) => t.kind === 'REVIEW' || t.kind === 'EXAM' || t.kind === 'LISTENING')));
  ...
  return (
    <div className="container tbp-view">
      <header className="tbp-header">
        <div>
          ...
          <p className="tbp-subtitle">
            Bài học trong app được xếp theo khung của từng bộ sách. Dùng sách song song, ghi số trang vào từng buổi.
          </p>
        </div>
      ...
      <div className="tbp-series" role="tablist" aria-label="Bộ giáo trình">
        {SERIES_ORDER.map((s) => {
          const m = JLPT_TEXTBOOK_SERIES[s];
          return (
            <button
              key={s}
              type="button"
              role="tab"
              aria-selected={s === series}
              className={`tbp-series__tab${s === series ? ' is-active' : ''}`}
              onClick={() => select(s, level)}
            >
      ...
      <div className="tbp-levels" role="tablist" aria-label="Cấp JLPT">
        {JLPT_MIND_LEVELS.map((lv) => {
          const available = PLAN_LEVELS[series].includes(lv);
          return (
            <button
              key={lv}
              type="button"
              ...
              disabled={!available}
              title={available ? undefined : `${meta.name} không có sách ${lv}`}
        ...
        <p className="tbp-books">
          ...
          {audio.length ? (
            ...
            <span className="tbp-muted">
              chưa có trong <Link href="/book-audio">File nghe sách</Link> cho {meta.name} {level}
      ...
      {!loading && lessonsError ? (
        <p className="tbp-empty">Không tải được danh sách bài học — kiểm tra API rồi thử lại.</p>
      ) : null}
      {!loading && !lessonsError && emptyPlan ? (
        <p className="tbp-empty">Chưa có bài học {level} trong app để xếp vào lộ trình này.</p>
      ) : null}
      ...
      {plan && !loading
        ? plan.sections.map((section) => (
            <section key={section.id} className="tbp-section">
              <h2>
                {section.title}
                {section.titleJa ? <span className="japanese-text"> {section.titleJa}</span> : null}
              </h2>
              <ol className="tbp-units">
                {section.units.map((unit) => {
                  const done = progress.done.includes(unit.id);
                  return (
                    <li key={unit.id} className={`tbp-unit glass-panel${done ? ' is-done' : ''}`}>
                      <label className="tbp-unit__head">
                        <input type="checkbox" checked={done} onChange={() => toggleDone(unit.id)} />
                        <span className="tbp-unit__title">{unit.title}</span>
                        {unit.subtitle ? <span className="tbp-unit__sub">{unit.subtitle}</span> : null}
                      </label>
                      <ul className="tbp-tasks">
                        {unit.tasks.map((task) => (
                          <li key={`${task.kind}-${task.href}`}>
                            <Link href={task.href}>
                              <span className="tbp-task__icon japanese-text" aria-hidden>
                                {TASK_ICON[task.kind]}
                              </span>
                              {task.label}
                            </Link>
      ...
      <p className="tbp-footnote">
        App không chứa nội dung sách (bản quyền NXB). Tiến độ và ghi chú lưu trên trình duyệt này.
      </p>
    </div>
  );
}
