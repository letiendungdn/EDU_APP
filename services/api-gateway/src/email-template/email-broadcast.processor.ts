...
export class EmailBroadcastProcessor extends WorkerHost {
  ...
  private async processSendTemplate(
    job: Job<SendTemplateJobData>,
  ): Promise<void> {
    const { broadcastId, userId, email, name, templateName, customVars } =
      job.data;
    ...
    try {
      ...
      if (templateName === "weekly_progress") {
        ...
        const [cardsReviewed, newMastered, totalMastered] = await Promise.all([
          ...
          this.prisma.srsCard.count({
            where: { userId, mastered: true, lastReviewedAt: { gte: since } },
          }),
      ...
      if (templateName === "streak_milestone") {
        ...
        const sampleMilestone =
          TEMPLATE_SAMPLE_VARS["streak_milestone"]?.["milestone"];
        const milestone = Number(
          customVars?.["milestone"] ?? sampleMilestone ?? 30,
        );
        Object.assign(baseVars, {
          milestone,
          currentStreak: streak,
      ...
      const { subject, html, text } = await this.emailTemplateService.preview(
        templateName,
        mergedVars,
      );
      ...
      this.logger.error(
        { err, email, templateName, broadcastId },
  ...
  private async processCompose(job: Job<ComposeJobData>): Promise<void> {
    ...
    try {
      ...
      this.logger.error(
        { err, email, broadcastId },
        "Compose broadcast job failed",
      );
      await this.updateCount(broadcastId, "failed");
    }
  }
