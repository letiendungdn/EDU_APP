...
| 2   | `postgres-english` | `edu-postgres-english` | **5434**           | Gateway có `EnglishModule` — **bắt buộc khởi động**, nhưng **không cần restore SQL English** nếu chỉ học JP |
| 3   | `redis`            | `edu-redis`            | 6379               | Cache, session, rate-limit                                                                                  |
| 4   | `mongodb`          | `edu-mongodb`          | 27017              | Audit log, notification                                                                                     |
| 5   | `zookeeper`        | `edu-zookeeper`        | —                  | Kafka cần                                                                                                   |
| 6   | `kafka`            | `edu-kafka`            | 9092               | Event thi thử / async                                                                                       |
| 7   | `content-service`  | `edu-content`          | — (gRPC 50051)     | Nội dung JP qua gRPC                                                                                        |
| 8   | `exam-service`     | `edu-exam`             | — (gRPC 50052)     | Mock JLPT, quiz                                                                                             |
| 9   | `api-gateway`      | `edu-gateway`          | **3000**           | REST API, auth, Swagger                                                                                     |
| 10  | `nihongo-web`      | `edu-nihongo-web`      | — (qua nginx)      | Frontend Next.js                                                                                            |
| 11  | `nginx`            | `edu-nginx`            | **8080**           | Cổng vào app: [http://localhost:8080](http://localhost:8080)                                                |
...
| ------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Chỉ DB/cache local** (dev bằng `npm run dev:`*) | `postgres-nihongo postgres-english redis mongodb zookeeper kafka`                                                                                         |
| **Học Nihongo (Docker full)**                     | `postgres-nihongo postgres-english redis mongodb zookeeper kafka content-service exam-service api-gateway nihongo-web nginx` — **không** có `english-web` |
| **+ Tiếng Anh**                                   | thêm `--profile english` hoặc `english-web`                                                                                                               |
