...
| **english-web** | `edu-english-web` | (qua nginx) | App học tiếng Anh. |
...
| Mục | URL |
|-----|-----|
| Nihongo (qua nginx) | http://localhost:8080 |
| English (qua nginx) | http://english.localhost:8080 *(cần hosts)* |
| API / Swagger | http://localhost:3000/api/docs |
| API qua nginx | http://localhost:8080/api/... |
| Signaling | http://localhost:3002 |
...
# Stack học tiếng Nhật (Docker full app, không english / monitoring)
...
| **Học Nihongo (Docker full)** | `postgres-nihongo postgres-english redis mongodb zookeeper kafka content-service exam-service api-gateway nihongo-web nginx` |
| **+ Tiếng Anh** | thêm `english-web` |
...
6. Mở http://localhost:8080 — login admin: `admin@nihongo.local` / `admin123` (đổi trong `.env`)
